...
def owner_dashboard():
    ...
    import logging as _ow_log

    from flask import current_app as _ow_current_app
    ...
    try:
        ...
        # Recent webhook activity â€” scoped to this owner's payment intents so that
        ...
        try:
            ...
            _ow_log = __import__('logging').getLogger('cafe.owner')

            _ow_log.exception('owner_dashboard degraded: %s', _ow_exc)

            try:

                from flask import flash as _ow_flash

                _ow_flash('Dashboard is in degraded mode -- some data failed to load. Refresh or check /health.', 'warning')
        ...
        from app.services.auth import logged_in_owner_obj as _ow_logged

        _fallback_owner = None

        try:

            _fallback_owner = _ow_logged()

        except Exception:

            pass

        from flask import render_template as _ow_render
        ...
        return _ow_render(
            ...
            integration_health={'configured':0,'payments_configured':0,'payments_live':0,'aggregators_configured':0,'aggregators_live':0,'counts':{'live':0,'ready':0,'unverified':0,'failing':0},'items':[]},
            ...
            owner_username=(_fallback_owner.username if _fallback_owner else ''),
            ...
            billing_net_today=0.0,

            billing_settled_today=0,
...
def owner_add_table():
    ...
    # â”€â”€ Plan limit gate â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def update_order_status(order_id: int):
    ...
    if _db_update_order_status(order_id, new_status):
        ...
        if new_status in ("completed","served","closed","paid"):
            try:
                from app.web.pos import loyalty_earn_for_order
                loyalty_earn_for_order(owner_id, order_id, getattr(order, 'customer_phone', '') or '', float(getattr(order, 'total', 0) or 0))
            except Exception:
                pass
...
# Reorder â€” look up past orders by customer phone
...
# Customers â€” aggregated customer ledger from order history
...
# Order receipt view (Bug #14 fix â€” port from legacy monolith)
...
def order_receipt(order_id: int):
    ...
    order_d = {
        ...
        "tableName": order.table_name or order.table_id or "â€”",
...
# Table bill shortcut (Bug #15 fix â€” port from legacy monolith)
...
# Order quick-actions (Bug fixes â€” singular URL form, ported from legacy)
...
def mark_order_paid(order_id: int):
    ...
    if order.status not in ("completed", "cancelled"):
        ...
        try:
            ...
            if _ord:
                loyalty_earn_for_order(owner_id, order_id, getattr(_ord, 'customer_phone','') or '', float(getattr(_ord,'total',0) or 0))
        except Exception:
            pass
...
# Analytics: day-orders â€” per-hour order counts and revenue for today
...
# Two-factor authentication setup / disable (Bug fixes â€” ported from legacy)
